<?php
//Se recogerían de la base de datos y se guardarían en variables para comparar
//aquí lo simulamos:
$usuarioBD = 'Isa';
$passBD = '1234';

//Recogemos el valor llegado de index:
$usuarioAplicacion = $_POST['usuario'] ?? null;
$passAplicacion = $_POST['password'] ?? null;

//Comparamos los introducidos con los de la BD:
if($usuarioAplicacion == $usuarioBD && $passAplicacion == $passBD ) {
    //Nos dejan entrar:
    session_start();
    $_SESSION['verificado'] = "si";
    echo "Tienes acceso a la página privada";
    echo "<a href='solo_miembros.php'>Accede al contenido sólo para miembros</a>";
}else{
    //El usuario no es correcto redirigimos a index:
